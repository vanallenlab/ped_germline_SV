...
export polished_vcf="gs://val-ped-nonterra-hail/PedSV.v2.3.polished_noCodingOutliers.vcf.gz"
...
hailctl dataproc submit \
  ...
  --vcf-in $polished_vcf \
  --pcs-out gs://val-ped-nonterra-hail/PedSV.v2.3.polished.PCs.tsv.gz \
  --kinship-out gs://val-ped-nonterra-hail/PedSV.v2.3.polished.kinship.tsv.gz
...
gsutil -m cp \
  gs://val-ped-nonterra-hail/PedSV.v2.3.polished.*.tsv.gz \
  $WRKDIR/data/ancestry_and_relatedness/

# Assign ancestries using top 10 PCs
if [ -e $WRKDIR/data/ancestry_and_relatedness/v2.3 ]; then
  rm -rf $WRKDIR/data/ancestry_and_relatedness/v2.3
fi
mkdir $WRKDIR/data/ancestry_and_relatedness/v2.3
${CODEDIR}/gatksv_scripts/assign_ancestry.R \
  --PCs $WRKDIR/data/ancestry_and_relatedness/PedSV.v2.3.polished.PCs.tsv.gz \
  ...
  --out-prefix $WRKDIR/data/ancestry_and_relatedness/v2.3/PedSV.v2.3.polished \
  --use-N-PCs 10 \
  --min-probability 0 \
  --plot
...
$CODEDIR/gatksv_scripts/parse_kinship.R \
  $WRKDIR/data/ancestry_and_relatedness/PedSV.v2.3.polished.kinship.tsv.gz \
...
> $WRKDIR/data/ancestry_and_relatedness/v2.3/PedSV.v2.3.samples_to_exclude_for_analysis_vcfs.list
