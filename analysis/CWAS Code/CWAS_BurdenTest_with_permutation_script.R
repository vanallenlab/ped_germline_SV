...
if(number_of_unique_SVs_cases + number_of_unique_SVs_controls > 10){
...
if(SV_count_vector_subsetted %>% length() > 0){
  ...
  SV_count_vector_subsetted_IQR <- SV_count_vector_subsetted %>% IQR()
  
  SV_count_vector_subsetted_median <- SV_count_vector_subsetted %>% median()
  
  if(!SV_count_vector_subsetted_IQR == 0){
  SV_count_vector_subsetted_bound_by_6IQR <- SV_count_vector_subsetted[(SV_count_vector_subsetted > (SV_count_vector_subsetted_median - 6*SV_count_vector_subsetted_IQR)) & (SV_count_vector_subsetted < (SV_count_vector_subsetted_median + 6*SV_count_vector_subsetted_IQR))]
  }
  else{
  SV_count_vector_subsetted_bound_by_6IQR <- SV_count_vector_subsetted
  }
  
  SV_counts_cases <- SV_count_vector_subsetted_bound_by_6IQR[sample_ids$cases][!is.na(SV_count_vector_subsetted_bound_by_6IQR[sample_ids$cases])] %>% sum()
  
  SV_counts_cases_max <- SV_count_vector_subsetted_bound_by_6IQR[sample_ids$cases][!is.na(SV_count_vector_subsetted_bound_by_6IQR[sample_ids$cases])] %>% max()
  
  number_of_cases_with_zero_SVs <- length(SV_count_vector_subsetted_bound_by_6IQR[sample_ids$cases][!is.na(SV_count_vector_subsetted_bound_by_6IQR[sample_ids$cases]) & SV_count_vector_subsetted_bound_by_6IQR[sample_ids$cases] == 0])
  
  total_cases <- SV_count_vector_subsetted_bound_by_6IQR[sample_ids$cases][!is.na(SV_count_vector_subsetted_bound_by_6IQR[sample_ids$cases])] %>% length()
  
  SV_counts_controls <- SV_count_vector_subsetted_bound_by_6IQR[sample_ids$controls][!is.na(SV_count_vector_subsetted_bound_by_6IQR[sample_ids$controls])] %>% sum()
  
  SV_counts_controls_max <- SV_count_vector_subsetted_bound_by_6IQR[sample_ids$controls][!is.na(SV_count_vector_subsetted_bound_by_6IQR[sample_ids$controls])] %>% max()
  
  number_of_controls_with_zero_SVs <- length(SV_count_vector_subsetted_bound_by_6IQR[sample_ids$controls][!is.na(SV_count_vector_subsetted_bound_by_6IQR[sample_ids$controls]) & SV_count_vector_subsetted_bound_by_6IQR[sample_ids$controls] == 0])
  
  total_controls <- SV_count_vector_subsetted_bound_by_6IQR[sample_ids$controls][!is.na(SV_count_vector_subsetted_bound_by_6IQR[sample_ids$controls])] %>% length()
  
  if((SV_counts_cases + SV_counts_controls) > 0){
  
  glm_vector <- pedsv.glm(gatk_sv_pediatric_cancers_combined_cohort_metadata_filtered_for_study_phase,SV_count_vector_subsetted_bound_by_6IQR,case_control_vector,family = binomial(), use.N.pcs = 3, extra.terms = "cohort")[1:4] %>% unlist() %>% unname()
  ...
  update_names <- names(SV_count_vector_subsetted_bound_by_6IQR) %>% sample()
  
  names(SV_count_vector_subsetted_bound_by_6IQR) <- update_names
  
  glm_results_permuted_z_score_matrix[[category_name,iteration]] <- pedsv.glm(gatk_sv_pediatric_cancers_combined_cohort_metadata_filtered_for_study_phase,SV_count_vector_subsetted_bound_by_6IQR,case_control_vector,family = binomial(), use.N.pcs = 3, extra.terms = "cohort")[[3]]
