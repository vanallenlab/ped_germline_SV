...
def trim_tree_by_similarities(g, node_df, node_positions, preserved_cats, jaccard, threshold, keep_metric, axis = 'y'):
    ...
    unique_vals = {pos: i for i, pos in enumerate(sorted(set(node_pos_df[axis]), reverse = True))}
    node_pos_df[f'{axis}_level'] = node_pos_df[axis].replace(unique_vals)
    ...
    nodes_to_remove = []
    for level in unique_vals.values():
        level_nodes = node_pos_df[node_pos_df[f'{axis}_level'] == level]
...
def tighten_tree(g, node_df, node_positions, threshold, keep_metric, axis = 'y'):
    ...
    # get the node levels
    node_pos_df = pd.DataFrame(node_positions, index = ['x', 'y']).T
    unique_vals = {pos: i for i, pos in enumerate(sorted(set(node_pos_df[other_axis]), reverse = True))}
    node_pos_df[f'{other_axis}_level'] = node_pos_df[other_axis].replace(unique_vals)
             
    # get the values of the nodes and their distances from each other
    vals = node_pos_df[axis].values
    difference_matrix = np.abs(vals[:, np.newaxis] - vals)
    ...
    already_tightened = []
    for level in unique_vals.values():
        level_nodes = node_pos_df[node_pos_df[f'{other_axis}_level'] == level]
        deeper_nodes = node_pos_df[node_pos_df[f'{other_axis}_level'] > level]

        for node in level_nodes.index:
            dist_deeper = diff_df.loc[node, deeper_nodes.index]

            # use some threshold
            dist_deeper_cats = list(dist_deeper[dist_deeper < threshold].index)

            # remove nodes that have already been tightened
            dist_deeper_cats = [c for c in dist_deeper_cats if c not in already_tightened]

            # average all of them
            pos = node_pos_df[axis].loc[[node] + dist_deeper_cats]
            tightened_pos = pos.mean()
            node_pos_df.loc[[node] + dist_deeper_cats, axis] = tightened_pos

            already_tightened += dist_deeper_cats
    ...
    for level in unique_vals.values():
        ...
        dups = level_nodes.duplicated(subset = axis, keep = False).values
        ...
        for val in set(overlapping[axis]):
            nodes = overlapping[overlapping[axis] == val]
            metric_values = node_df.loc[nodes.index].sort_values(by = keep_metric, ascending = False)
